...
# ╔═╡ 8809765d-4a1c-4ee9-90d9-ac4c13944b16
using PlutoUI, ControlSystems, Plots

# ╔═╡ ae55cdc0-193a-11f0-1a6a-0ff4dbf78d21
md"""
# Goby's Control Systems Toolbox

Welcome to the **fundamentals** notebook of the **Goby's Control Systems Toolbox**, an interactive control systems toolkit!

This notebook covers the foundations of control theory with live simulations and plots.  
You'll explore:

- **Open-Loop Systems**  
- **Closed-Loop Systems**  
- **System Responses & Analysis Tools**
- **Controllers Design & Tunings**

Everything here is built to help you **learn by doing** — tweak, simulate, and see what happens.

---

📜 **License**: Beerware 🍻  
> Copyright © 2025 goby98
>  
> As long as you retain this notice, you can do whatever you want with this stuff.
>
> If we meet someday and you think this project helped you,  
> you can buy me a beer in return.  
>  
> 🍊 *Or orange juice. Beer isn’t great for your liver anyway.*

---
"""


# ╔═╡ 1b4dbe65-220e-4436-bab1-c9888aa0d600
md"""
# Transfer Function Designer

Definitions:
- **P**: The plant (system to be controlled)
- **C**: The controller
- **H**: The feedback

### Open-Loop
Transfer function: ``G\left( s \right) = C\left( s \right) \cdot P\left( s \right)``

### Closed-Loop
Transfer Function: ``T\left( s \right) = \frac{C\left( s \right) \cdot P\left( s \right)}{1 + C\left( s \right) \cdot P\left( s \right) \cdot H\left( s \right)}``
"""

# ╔═╡ ca7d3f96-b94d-432f-a5e0-3970f464f5fe
begin
    md"""
    ## Controller Function
    
    C(s): $(@bind C_num_str TextField(default="1 2 3"))
    / $(@bind C_den_str TextField(default="1 2 3"))
    """
end

# ╔═╡ 8daf5ca1-f90f-4b84-b2a4-3ca23e573a51
begin
	# Parse input strings into arrays of Floats
	C_num = parse.(Float64, split(C_num_str))
	C_den = parse.(Float64, split(C_den_str))
	
	# Create transfer functions
	C = tf(C_num, C_den)
end

# ╔═╡ b4886a0b-b2c0-4315-8189-5ac2bd6083cd
begin
    md"""
    ## Plant Function

    P(s): $(@bind P_num_str TextField(default="1 2 3"))
    / $(@bind P_den_str TextField(default="1 2 3"))
    """
end

# ╔═╡ d57efce7-1652-4787-ae4f-bb1c07d0eccd
begin
	# Parse input strings into arrays of Floats
	P_num = parse.(Float64, split(P_num_str))
	P_den = parse.(Float64, split(P_den_str))
	
	# Create transfer functions
	P = tf(P_num, P_den)
end

# ╔═╡ 5bb67147-9868-4a4f-a3ed-03021c788644
md"""
## System
"""
...
md"""
System Type: $(@bind system_type Select(["Open-Loop", "Closed-Loop"]))
"""

# ╔═╡ c8979555-c61d-4802-a70a-248727b76443
if system_type == "Closed-Loop"
    md"""
    H(s): $(@bind H_num_str TextField(default="1 2 3"))
    / $(@bind H_den_str TextField(default="1 2 3"))
    """
end
...
begin
	if system_type == "Closed-Loop"
    	H_num = parse.(Float64, split(H_num_str))
    	H_den = parse.(Float64, split(H_den_str))
    	H = tf(H_num, H_den)
	end
end
...
You can choose different types of inputs to test the system response. The available input functions are: **Step**, **Ramp**, **Impulse**, **Pulse**, **Sinusoidal**, **Cosine**.
"""

# ╔═╡ 4c001fd9-7c70-4ff2-95bb-cb9b0f106c39
md"""
Function: $(@bind harmonic_function_type Select(["Step", "Ramp", "Impulse", "Pulse", "Sinusoidal", "Cosine"]))
"""

# ╔═╡ 54e70af5-b472-4f81-b736-792686173460
# End time
if harmonic_function_type != "Step"
	md"""
	Time end: $(@bind t_max_input Slider(1:1:25, default=10))
	"""
end

# ╔═╡ 6b4b33cf-05d4-41bc-bbac-63221669415b
# End time print
if harmonic_function_type != "Step"
	md"""
	Time end: $(t_max_input) sec
	"""
end

# ╔═╡ c7958a2d-2074-4fdf-be13-933e0232ceb5
# Ramp
if harmonic_function_type == "Ramp"
	md"""
	Slop gain:  $(@bind slop_input TextField(default="1"))
	"""
# Pulse
elseif harmonic_function_type == "Pulse"
	md"""
	Pulse gain:  $(@bind pulse_gain_input TextField(default="1"))
	
	Pulse width: $(@bind pulse_width_input TextField(default="1"))
	
	Pulse period: $(@bind pulse_period_input TextField(default="1"))
	"""
	
# Sinusoidal
elseif harmonic_function_type == "Sinusoidal" || harmonic_function_type == "Cosine"
	md"""
	Amplitude:  $(@bind harmonic_amplitude_input TextField(default="1"))
	
	Frequecy: $(@bind harmonic_frequency_input TextField(default="1"))
	"""
end

# ╔═╡ 2e575076-5b3e-4934-b7bf-4c43e233382b
begin
	# Discretize the system using c2d
	if harmonic_function_type != "Step"
		Ts = 0.001 # Time step
		time_vec = 0:Ts:t_max_input # Create a time vector from 0 to t_max_input
	    sys_d = c2d(G, Ts)
	end
	...
	elseif harmonic_function_type == "Ramp"
		# Parse input values
		slop_val = parse(Float64, slop_input)
		
		ramp(t) = slop_val * t  # Ramp with a slope of K
		...
		plot(lsim(sys_d, ramp_input_matrix, time_vec)) # Plot
		
	# Impulse
	elseif harmonic_function_type == "Impulse"
		plot(impulse(G, t_max_input))
		
	# Pulse
	elseif harmonic_function_type == "Pulse"
		# Parse input values
		pulse_gain_val = parse(Float64, pulse_gain_input)
		pulse_width_val = parse(Float64, pulse_width_input)
		pulse_period_val = parse(Float64, pulse_period_input)
		
		pulse(t) = pulse_gain_val * (t >= pulse_period_val && t <= pulse_period_val + pulse_width_val)  # Pulse
		
		# Generate pulse input for each time step and reshape as a matrix (1 row x N columns)
		pulse_input = [pulse(t) for t in time_vec]
		pulse_input_matrix = reshape(pulse_input, 1, length(pulse_input))

		# Plot the system response to the pulse input using lsim
      	plot(lsim(sys_d, pulse_input_matrix, time_vec))

	# Sinusoidal or Cosine
	elseif harmonic_function_type == "Sinusoidal" || harmonic_function_type == "Cosine"
	    # Parse input values
	    harmonic_amplitude_val = parse(Float64, harmonic_amplitude_input)
	    harmonic_frequency_val = parse(Float64, harmonic_frequency_input)
	
	    if harmonic_function_type == "Sinusoidal"
	        harmonic_func(t) = harmonic_amplitude_val * sin(2 * pi * harmonic_frequency_val * t)  # Sinusoidal input
	    elseif harmonic_function_type == "Cosine"
	        harmonic_func(t) = harmonic_amplitude_val * cos(2 * pi * harmonic_frequency_val * t)  # Cosine input
	    end
	
	    # Generate harmonic input for the discrete-time system
	    harmonic_input = [harmonic_func(t) for t in time_vec]
	
	    # Reshape harmonic input to match the expected input format for lsim
	    harmonic_input_matrix = reshape(harmonic_input, 1, length(harmonic_input))
	
	    # Plot the system response to the harmonic input using lsim
	    plot(lsim(sys_d, harmonic_input_matrix, time_vec))  # Plot
	end
end

# ╔═╡ 0f9339fd-a6a3-4bbe-8ecb-5be14bb079d1
md"""
# Basic System Analysis

Tools for analyzing your system: **Poles**, **Zeros**, **Step Response Info**
"""

# ╔═╡ 8c53b8be-ec63-4cef-96ce-c601abf73fee
md"""
## System Poles
"""

# ╔═╡ 3f903e5f-832f-4c77-8025-cbc955ef7c4a
pole(G)

# ╔═╡ 809ef93d-044f-4185-ae11-9fa9ffd67e16
md"""
## System Zeros
"""

# ╔═╡ a422d287-0b90-47f5-9599-2c6bd75de301
tzeros(G)

# ╔═╡ 90e2afbf-3005-4878-868d-6d82f505e8e5
md"""
## Stepinfo
"""
...
Plots: **Bode**, **Nyquist**, **Root Locus**, **Pole-Zero Map**, **Nichols Chart**
"""

# ╔═╡ 9f170526-a08e-462f-a01a-c85717b63f13
md"""
Plot: $(@bind plot_type Select(["Bode", "Nyquist", "Root Locus", "Pole-Zero Map", "Nichols Chart"]))
"""

# ╔═╡ 618ac184-a8d2-4a88-acef-6d449503eb51
if plot_type == "Bode"
	bodeplot(P)
elseif plot_type == "Nyquist"
	nyquistplot(P)
elseif plot_type == "Root Locus"
	rlocusplot(P)
elseif plot_type == "Pole-Zero Map"
	pzmap(P)
elseif plot_type == "Nichols Chart"
	nicholsplot(P)
end
...
git-tree-sha1 = "6895024a7881b5567d6f122c276ac3048f28160d"
